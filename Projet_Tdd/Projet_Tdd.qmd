...
Le traitement des données « sales » dans les fichiers CSV réels, comme les valeurs manquantes (NaN) ou les formats incohérents (poids exprimés en "lbs" au lieu de simples nombres), a nécessité une attention particulière. Nous avons résolu ce problème en intégrant des mécanismes de nettoyage et de conversion directement dans les classes *StatistiqueJoueur* et *ServiceAnnuaireJoueurs*, assurant la transformation systématique des données (conversion en float ou passage des lbs aux kg) et le filtrage des valeurs inexploitables.
